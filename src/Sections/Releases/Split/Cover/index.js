...
const Cover = Keyframes.Spring({
  open: async (next, cancel, { onRest }) => {
    ...
    next({
      sB: 0,
      from: { sB: 100, sC: 100, mO: 0, mT: 5, pC: 0, cL: 0, bT: 0 }
    });
    await delay(300);
    next({ sC: 0 });
    await delay(500);
...
const ReleaseSplitCover = ({ stage, onRest, displayStory }) => (
  <Cover native state={stage} onRest={onRest}>
    {({ sB, sC, mO, mT, pC, cL, bT }) => (
      <Fragment>
        <animated.div
          className={classNames('split-container split__cover', {
            'split__cover--story': displayStory
          })}
          style={{
        ...
        >
          ...
          <animated.div
            ...
            style={{
              clipPath: pC.interpolate(
                p => `polygon(0 0, 0% 100%, ${p}% 100%, ${p}% 0)`
              )
            }}
          >
            <img src={ImgBkgd} alt="" />
